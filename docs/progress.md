...
## M5 - Attack bench: COMPLETE

24 scenarios across 8 attack classes, three variants each, run under both configurations.

| Configuration | Attack success | Utility |
|---|---|---|
| Unprotected agent | **100%** (24/24) | 100% |
| With Hallmark | **0%** (0/24) | 100% |

Success is judged on the ledger and the outbox, never on what the agent said. Full results
and caveats in `docs/bench-results.md`, regenerated by `scripts/run_bench.py`; the console
renders the same generated data.

**Two measurement faults were found and fixed before publishing.** The exfiltration
scenarios carried invoice fields, so the agent took the invoice path and never attempted
the export -- which the bench scored as a successful defence. And baseline exfiltration
success was hardcoded rather than measured. Both would have inflated the result.

A test now asserts the unprotected agent actually attempts every scenario, because an
attack the agent cannot reach measures the fixture rather than the defence.
